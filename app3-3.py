...
def get_llm(model_name: str):
    if model_name == "qwen2-7b":
        return Ollama(model="qwen2", base_url="http://localhost:11434", verbose=True)
...
def get_llm_chain(prompt_context, model_name):
...
def initialize_session_state():
    ...
    if "llm_chain" not in st.session_state:
        st.session_state["llm_chain"] = get_llm_chain(None, None)
    ...
    with st.sidebar:
        ...
        selected_model = st.sidebar.selectbox('Choose a LLM model', ['Llama3-8B', 'qwen2', 'Gemini-1.5'], key='selected_model')
